...
try:
	print("Processando arquivo csv -> "+sys.argv[1])
	fileName = sys.argv[1]
except:
	print("usage: filter_csv arquivo_de_entrada.csv")
	exit()
...
with codecs.open(fileName,'r',encoding='iso8859-1') as f:
	for line in f:
		...
		if {values[32]}.issubset(CIDs):
			qtd_cid_principal += 1
			#print("->("+values[7]+"-"+values[8]+"-"+values[11]+"): é um CID Principal contido em CIDs relacionadas a 'parto e puerpérias'; ["+values[32]+"]")

		if {values[33]}.issubset(CIDs):
			qtd_cid_secundario += 1
			#print("->("+values[7]+"-"+values[8]+"-"+values[11]+"): é um CID Secundário contido em CIDs relacionadas a 'parto e puerpérias'; ["+values[33]+"]")

print("\tEsse arquivo possui "+str(qtd_cid_principal)+" registros CID Principal associados a 'parto e puerpérias'")
print("\tEsse arquivo possui "+str(qtd_cid_secundario)+" registros CID Secundário associados a 'parto e puerpérias'")
